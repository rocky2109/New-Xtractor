    ...
    try:
        ...
        for i in range(start_index - 1, len(links)):
            ...
            try:
                ...
                audio_title = f"YouTube_Video_{count}"

            audio_title = sanitize_filename(audio_title)
            base_name = f'{str(count).zfill(3)} - {audio_title} {CREDIT}'.strip()
            output_file = f"{base_name[:60]}.mp3"

            if "youtube.com" in url or "youtu.be" in url:
                prog = await m.reply_text(
                    f"<i><b>Audio Downloading</b></i>\n<blockquote><b>{base_name}</b></blockquote>"
                )

                cmd = f'yt-dlp -x --audio-format mp3 --cookies {cookies_file_path} "{url}" -o "{output_file}"'
                ...
                if os.path.exists(output_file):
                    ...
                    try:
                        ...
                        await bot.send_document(
                            chat_id=m.chat.id,
                            document=InputFile(output_file, filename=f"{base_name}.mp3"),
                            caption=(
                               f'🎵 **Title**: [{str(count).zfill(3)}] - {audio_title}\n\n'
                               f'🔗 **Video link**: {url}\n\n'
        ...
        await m.reply_text(f"<b>⚠️ Error Occurred:</b>\n<blockquote><code>{str(e)}</code></blockquote>")
    finally:
        await m.reply_text("<blockquote><b>✅ All Downloads Complete.</b></blockquote>")
